...
CochraneClassicalMetaAnalysis   <- function(jaspResults, dataset, options, state = NULL) {
  ...
  if (is.null(jaspResults[["database"]]))
    .loadDatabase(jaspResults)
  
  # select data
  if (is.null(jaspResults[["dataset"]]))
    .selectData(jaspResults, options)
  
  dataset <- jaspResults[["dataset"]]$object
  
  # overview table
  if (is.null(jaspResults[["selectedOverviewTable"]]))
    .selectedOverviewTable(jaspResults, options)
  
  # overview figures
  if (options[["plotEffectSizes"]])
    .decriptivePlot(jaspResults, dataset, "effectSize")
  
  # overview figures
  if (options[["plotSampleSizes"]])
    .decriptivePlot(jaspResults, dataset, "sampleSize")
  
  # reusing classical meta-analysis
  options <- .emulateMetaAnalysisOptions(options)
  
  
  ready <- .readyKeywordsOrTopicsOrSearch(options)
  if (ready) {
    if (nrow(dataset) == 0)
      ready      <- FALSE
    else{
      effectSize <- dataset$effectSize
      effectSE   <- dataset$effectSE 
    }
  }
  ...
  #ClassicalMetaAnalysis(jaspResults, dataset, options)
  
  return()
}

.loadDatabase          <- function(jaspResults){
...
.selectData            <- function(jaspResults, options){
  
  # create a notifier for updating the dataset
  dataset <- createJaspState()
  dataset$dependOn(c("selectionType", "topicsSelected", "keywordsSelected", "textSearch"))
  jaspResults[["dataset"]] <- dataset
  
  # create an object for storing selected DOIs
  if (is.null(jaspResults[["selectedDoi"]])) {
    selectedDoi <- createJaspState()
    jaspResults[["selectedDoi"]] <- selectedDoi
  } else
    selectedDoi <- jaspResults[["selectedDoi"]]
  ...
  if (options[["selectionType"]] == "selectionTopics") {
    ...
    doiTopics <- unname(unlist(sapply(database[["metaAnalyses"]], function(metaAnalysis){
      if (metaAnalysis[["topic"]] %in% options[["topicsSelected"]])
        return(metaAnalysis[["doi"]])
      else
        return(NULL)
    })))
    
    selectedDoi$object <- doiTopics
    dataset$object     <- database[["studies"]][database[["studies"]][["doi"]] %in% doiTopics, ]
    
    return()
    
  } else if (options[["selectionType"]] == "selectionKeywords") {
    
    doiKeywords <- unname(unlist(sapply(database[["metaAnalyses"]], function(metaAnalysis){
      if (any(metaAnalysis[["keywords"]] %in% options[["keywordsSelected"]]))
        return(metaAnalysis[["doi"]])
      else
        return(NULL)
    })))
    
    selectedDoi$object <- doiKeywords
    dataset$object     <- database[["studies"]][database[["studies"]][["doi"]] %in% doiKeywords, ]
    ...
    metaAnalyses       <- database[["metaAnalyses"]]
    metaAnalysesTitles <- tolower(unname(sapply(metaAnalyses, function(metaAnalysis)metaAnalysis[["title"]])))
    
    textSearchPositive <- apply(matrix(sapply(searchPositive, function(text){
      return(grepl(text, metaAnalysesTitles, fixed = TRUE))
    ...
    if (length(textSearchNegative) == 0)
      doiSearch <- sapply(metaAnalyses, function(metaAnalysis)metaAnalysis[["doi"]])[textSearchPositive]
    else
      doiSearch <- sapply(metaAnalyses, function(metaAnalysis)metaAnalysis[["doi"]])[textSearchPositive & !textSearchNegative]      
    
    
    selectedDoi$object <- doiSearch
    dataset$object     <- database[["studies"]][database[["studies"]][["doi"]] %in% doiSearch, ]
    
    return()
    
  }
}
.selectedOverviewTable <- function(jaspResults, options){
  
  metaAnalyses         <- jaspResults[["database"]]$object[["metaAnalyses"]]
  selectedDoi          <- jaspResults[["selectedDoi"]]$object
  selectedMetaAnalyses <- metaAnalyses[selectedDoi]
  ...
  selectedOverviewTable$dependOn(c("selectionType", "topicsSelected", "keywordsSelected", "textSearch"))
  jaspResults[["selectedOverviewTable"]] <- selectedOverviewTable
  
  
  for (selectedMetaAnalysis in selectedMetaAnalyses)
    selectedOverviewTable$addRows(with(selectedMetaAnalysis, list(
      name      = title,
      year      = year,
      nStudies  = nStudies
    )))
...
.emulateMetaAnalysisOptions <- function(options){
...
.readyKeywordsOrTopicsOrSearch <- function(options){
  if (options[["selectionType"]] == "selectionTopics")
    return(length(options[["topicsSelected"]]) > 0)
  else if (options[["selectionType"]] == "selectionKeywords")
    return(length(options[["keywordsSelected"]]) > 0)
  else if (options[["selectionType"]] == "selectionTextSearch")
    return(nchar(options[["textSearch"]]) > 0)
}
.decriptivePlot             <- function(jaspResults, dataset, variable){
  
  if (!is.null(jaspResults[[paste0(variable,"Plot")]]))
    return()
  ...
  descriptivePlot <- createJaspPlot(
    ...
    dependencies = c("selectionType", "topicsSelected", "keywordsSelected", "textSearch", if(variable == "effectSize") "plotEffectSizes" else "plotSampleSizes"))
  
  jaspResults[[paste0(variable,"Plot")]] <- descriptivePlot
...
if(FALSE){
  ...
  options$topicsSelected  <- "Back and Neck"
}
...
.metaAnalysisGetOutputContainer <- function(jaspResults) {
  if (!is.null(jaspResults[["modelContainer"]])) {
    ...
    modelContainer$dependOn(c("topicsSelected", "keywordsSelected", "textSearch", "method", "studyLabels", "covariates", "test",
